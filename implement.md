...
- **Type inference**: `x = 10` infers `int`, `s = "hello"` infers `string`
- **Type annotations**: `int x = 10;` as optional typed declaration
- **Null / none type**: distinguish missing value from empty string. `string? x = none;`
...
- **Ternary**: `x = cond ? a : b;`
- **Null coalescing**: `x = maybe ?? "default";`
...
- Action return on failure: `result = risky() or exit(1);`
...
nums = [1, 2, 3, 4, 5];
evens = nums.filter(action(n) { return n % 2 == 0; });
```
